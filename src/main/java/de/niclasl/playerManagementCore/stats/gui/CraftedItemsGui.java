package de.niclasl.playerManagementCore.stats.gui;
...
public record CraftedItemsGui(PlayerManagementCore plugin) {
    ...
    public void open(Player viewer, OfflinePlayer target, int page) {
        ...
        int startIndex = (page - 1) * craftedItemsPerPage;
        int endIndex = Math.min(startIndex + craftedItemsPerPage, craftedItems.size());

        int slotIndex = 0;

        for (int i = startIndex; i < endIndex; i++) {
            Material material = craftedItems.get(i);
            int count = target.getStatistic(Statistic.CRAFT_ITEM, material);
            ...
            meta.setLore(List.of("§7Craft: §f" + count));
        ...
        viewer.setMetadata("craft_target", new FixedMetadataValue(plugin, target.getUniqueId().toString()));
        viewer.setMetadata("craft_page", new FixedMetadataValue(plugin, page));
    }
